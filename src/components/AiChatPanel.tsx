...
export default function AiChatPanel({ activeAthleteEmail, activeAthleteName }: Props) {
  ...
  /* Aprobar, rechazar y comentar: la misma lógica que usa la pantalla de
     Propuestas (src/components/ai/useProposalActions.ts). Antes vivía aquí
     dentro, y la pantalla nueva habría tenido que duplicarla. */
            ...
            <div className="border-t border-amber-500/20 bg-amber-500/5 p-3 flex flex-col gap-2 max-h-[55%] overflow-y-auto">
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                {lista.length > 1 && (
                  ...
                  >
                    <Icon name={acciones.aprobandoTodas === email ? 'progress_activity' : 'check_circle'} size="s" className={acciones.aprobandoTodas === email ? 'animate-spin' : ''} />
              ...
              {lista.map((p, idx, todas) => (
                <ProposalCard
                  ...
                  onAbrirEnEditorDeDietas={abrirEnEditorDeDietas}
                />
              ))}
              </React.Fragment>
              ))}
