...
function ZonesTab({ coachEmail }: { coachEmail: string }) {
  ...
  return (
    <section className="bg-surface border border-hairline rounded-2xl p-4 sm:p-5 space-y-2">
      <h2 className="font-sans font-bold text-base text-white mb-2">Elige un atleta</h2>
      {athletes.map(a => (
        <button key={a.email} onClick={() => setSelected(a.email)} className="w-full flex items-center gap-3 bg-raised border border-hairline rounded-lg p-3 hover:border-accent/40 transition-colors">
    ...
    <section className="bg-surface border border-hairline rounded-2xl p-4 sm:p-5 space-y-4">
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          ...
          <input type="number" value={restingHR} onChange={e => setRestingHR(e.target.value)} className="w-full bg-bg border border-hairline rounded p-2 text-xs text-white focus:outline-none focus:border-accent" />
        </div>
        <div className="flex-1">
          <label className="text-[10px] font-mono uppercase text-ink-2">FCmax</label>
          <input type="number" value={maxHR} onChange={e => setMaxHR(e.target.value)} className="w-full bg-bg border border-hairline rounded p-2 text-xs text-white focus:outline-none focus:border-accent" />
        </div>
        ...
        {ZONE_ORDER.map(z => (
          <div key={z} className="flex items-center gap-2">
            ...
            <input type="number" value={active[z].min} onChange={e => setZones({ ...active, [z]: { ...active[z], min: Number(e.target.value) } })}
              className="w-20 bg-bg border border-hairline rounded p-1.5 text-xs text-white focus:outline-none focus:border-accent" />
  ...
  return (
    <section className="bg-surface border border-hairline rounded-2xl p-4 sm:p-5 space-y-2">
      ...
      {tests.length === 0 ? (
        ...
        const athlete = profiles.find(p => p.email === t.athleteId);
        return (
          <div key={t.id} className="bg-raised border border-hairline rounded-lg p-3 space-y-2">
  ...
  return (
    <section className="bg-surface border border-hairline rounded-2xl p-4 sm:p-5 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        {type !== 'intervalos' && (
          <input type="number" value={durationMin} onChange={e => setDurationMin(e.target.value)} placeholder="Min" className="w-20 bg-bg border border-hairline rounded p-2 text-xs text-white focus:outline-none focus:border-accent" />
        )}
        <input type="number" value={timesPerWeek} onChange={e => setTimesPerWeek(e.target.value)} placeholder="x/sem" className="w-20 bg-bg border border-hairline rounded p-2 text-xs text-white focus:outline-none focus:border-accent" />
      </div>

      {type === 'intervalos' && (
        <div className="space-y-2 bg-bg border border-hairline rounded-xl p-3">
          ...
          {blocks.map((b, i) => (
            <div key={i} className="flex gap-1.5 items-center">
              <input value={b.label} onChange={e => updateBlock(i, { label: e.target.value })} placeholder={`Bloque ${i + 1}`}
                className="flex-1 min-w-0 bg-surface border border-hairline rounded p-1.5 text-[10px] text-white focus:outline-none focus:border-accent" />
              <input type="number" min={5} value={b.durationSec} onChange={e => updateBlock(i, { durationSec: Number(e.target.value) })}
                className="w-14 bg-surface border border-hairline rounded p-1.5 text-[10px] text-white focus:outline-none focus:border-accent" />
              ...
              <select value={b.targetZone} onChange={e => updateBlock(i, { targetZone: e.target.value as keyof CardioZones })}
                className="bg-surface border border-hairline rounded p-1.5 text-[10px] text-white focus:outline-none focus:border-accent">
