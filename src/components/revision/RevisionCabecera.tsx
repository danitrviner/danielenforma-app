...
interface Props {
  ...
  /** Adherencia combinada 28 días del Hub. `null` si no hay datos suficientes. */
...
function Cifra({ label, valor, sub, tono = 'ink' }: {
  ...
  return (
    <div className="bg-surface border border-hairline rounded-surface px-4 py-3 flex-1 min-w-[132px]">
...
export default function RevisionCabecera({
  athlete, ventana, informe, adherenciaPct = null, faseNutricional = null,
}: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Cifra
          label="Adherencia"
          valor={adherenciaPct != null ? `${adherenciaPct}%` : '—'}
          sub={adherenciaPct != null ? 'últimos 28 días' : 'sin datos'}
          tono={adherenciaPct != null ? 'ink' : 'atenuado'}
        />
        <Cifra
          label="Récords"
