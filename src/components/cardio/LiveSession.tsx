  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-5 pt-[max(env(safe-area-inset-top),1rem)]">
        ...
        <p className="text-xs font-mono text-white/80 tabular-nums">{fmtClock(elapsedSec)}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-3 space-y-4">
        <div className="text-center pt-2">
          ...
          <p className="text-xs font-mono uppercase text-white/90 mt-1.5 tracking-wider">{zoneLabel}</p>
        </div>
        ...
        {intervalBlocks && currentBlockIndex !== undefined && (
          <div className="bg-black/25 rounded-surface p-3.5 space-y-2 text-center">
            ...
            {intervalBlocks[currentBlockIndex + 1] && (
              <p className="text-[9px] font-mono text-white/60">Siguiente: {intervalBlocks[currentBlockIndex + 1].label}</p>
            )}
          </div>
        )}
