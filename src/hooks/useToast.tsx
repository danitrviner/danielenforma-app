...
const KIND_STYLE: Record<ToastKind, { icon: string; classes: string }> = {
  ...
  info:    { icon: 'cloud_sync',   classes: 'bg-raised text-ink border border-strong' },
};
    ...
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 items-center px-4 w-full max-w-md pointer-events-none">
        {toasts.map(t => (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-center gap-2 px-4 py-2.5 rounded-surface shadow-lg font-sans text-body-s font-bold w-full ${KIND_STYLE[t.kind].classes}`}
          >
            <span className="material-symbols-outlined text-title-s flex-shrink-0">{KIND_STYLE[t.kind].icon}</span>
